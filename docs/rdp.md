...
- **Sample and combine.** `interpolate_st` gives the per-pixel coordinate (the **non-perspective**
  ...
  **base tile** (`(hi >> 16) & 7`, bits 50:48) with `fetch_texel` and runs the combiner (with any
  shade). Works standalone and with shade/depth.
...
tile** is threaded from the triangle command (bits 50:48) into the sampler — `tiles[base_tile]` /
`tiles[(base_tile + 1) & 7]`, not a hardwired tile 0/1 — validated by `tex_tri_base_tile_16` (the
ramp loaded into tile 3 renders identically to tile 0; a `tiles[0]` read renders black). Scope
