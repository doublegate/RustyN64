...
| n64-systemtest `Failed: 0` (**CPU/COP0/TLB** — Phase 1's criterion) | **yes** — ROM committed | **runs; 1 failing in those categories** (414 suite-wide) — `BC1F`/`BC1T` are unimplemented (ledger R-2). The RSP's 291 are **Phase 2's** criterion, not this one |
