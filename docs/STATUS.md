...
frame for most titles (ledger **R-18** — though Paper Mario does render, so the gap is per-title coverage rather than the pipeline); the playable path is
