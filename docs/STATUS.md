...
| Accuracy battery (`AccuracyScorer::default_battery`) | **yes** — 48 probes across two oracle suites: 35 Angrylion RDP rasteriser vectors + 13 Angrylion VI scan-out vectors (expected values are the oracle's, never our own output) | **100% (48/48)** — asserted by `default_battery_matches_the_oracle`; both suites are asserted to contribute, and an empty battery now scores 0%, not a vacuous 100% |
