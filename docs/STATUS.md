...
| n64-systemtest `Failed: 0` (**CPU/COP0/TLB** — Phase 1's criterion) | **yes** — ROM committed | **`Failed: 0` in those categories** (413 suite-wide; the remainder are RSP/RCP, which is Phase 2's criterion). The RSP's 291 are **Phase 2's** criterion, not this one |
