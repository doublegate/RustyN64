...
| --- | --- | --- | --- | --- |
| R-14 | **CONFIRMED BUG (pending fix)** — `triangle_fill` renders triangle edges **4× too steep/fast**: a slope that should widen a triangle 0.25 px per pixel-row widens it 1 px per row. Found by the T-33-005 conformance gate on its first triangle vector — Angrylion renders a near-vertical line, RustyN64 a staircase | The edge slopes `DxHDy`/`DxMDy`/`DxLDy` are **dx per pixel-row** (N64brew *…/Commands* §Edge Coefficients: "change in x per change in y", with `yh/ym/yl` in `s11.2` *screen* pixels). `triangle_fill` evaluates the edge at each sub-scanline as `xh + (y − yh_base) · dxhdy` where `y = line·4 + sub` is in **quarter-pixel** units, but never divides the contribution by 4 — so each edge advances 4×. parallel-rdp compensates by pre-shifting the slope (`span_setup.comp:167`: `setup.dxhdy = raw >> 2`, then multiplies by quarter-units); RustyN64 uses the raw slope. Applying `>> 2` to the three decoded slopes makes the `fill_tri_16` conformance vector pass byte-for-byte (verified experimentally) | absolute — a fixed-point unit error, oracle-confirmed, not a fitted constant | **Open — the fix is a follow-up.** The `fill_tri_16` conformance vector (`tests/rdp_conformance.rs`) is committed and `#[ignore]`d to pin the bug. The self-asserted `fill_triangle_flat_fills_a_right_triangle` unit test baked in the buggy staircase (a circular golden — its expected pixels were never checked against an oracle), so the fix must **re-derive the affected triangle unit-test goldens against Angrylion** and un-ignore the vector. Does not change the n64-systemtest count (no systemtest drives the render path); the oracle stays **93** |
