...
| R-2 | `Comparisons in half mode with odd register indices` reports `different02 = 1` where 0 is expected | **Two causes, one now fixed.** (1) `BC1F`/`BC1T` were not implemented at all — the branch decoded to `Cop1Unimplemented` and retired as a no-op. All four forms are now decoded and executed. (2) What remains is the hazard this entry originally guessed at: `BC1` reads `FCSR.C` in `EX`, `C.cond.fmt` writes it at the single commit-or-trap point in `WB` (ADR 0007), and the ROM emits the pair with no separating instruction, so the branch samples the previous condition | absolute | **Open.** A first interlock attempt — stall while a compare sits in `ex_dc` or `dc_wb` — fires exactly once and is then satisfied while the commit still has not happened, so the branch runs two cycles early anyway. A per-cycle trace shows the condition becoming visible at cycle 5 for a compare that entered `EX` at cycle 1, which does **not** match the 3-cycle `EX→WB` distance; something else (an FP multi-cycle stall) is extending it. Resolve by tracing where the compare actually is on each of those cycles — **not** by picking a stall count that makes the ROM pass, which would be a fitted timing constant (`docs/accuracy-ledger.md` §1) |
