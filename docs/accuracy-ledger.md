...
| R-18 | A **commercial ROM boots and executes real code but does not reach video** (the Phase 5 capstone). | Recorded at the time as an un-modelled retail OS-boot runtime; the actual causes proved to be an `ERET` interrupt livelock in the CPU and a PIF that reported four controllers on a one-pad console. | absolute — a coverage boundary across subsystems, not a fitted constant or a timing interval | **Capstone met.** Super Mario 64 renders its title screen and 29 of 66 staged titles render through their own microcode. 24 titles still issue zero RDP commands — they *halt*, each for its own reason, and each is a separate root-cause hunt. **Full record: [R-18](residuals/R-18.md).** |
