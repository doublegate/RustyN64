...
| R-2 | `Comparisons in half mode with odd register indices` reports `different02 = 1` where 0 is expected | **`BC1F`/`BC1T` are not implemented.** Not a pipeline hazard — that was this entry's previous diagnosis and it was wrong. A per-cycle trace shows the branch decoding to `Cop1Unimplemented` and retiring as a no-op, so the flag the test reads is simply never overwritten. `pipeline.rs` says so in as many words: *"a real, valid COP1 branch that is genuinely not wired yet"*, with a note to move the test when `BC1` lands. The earlier reading — `FCSR.C` commits in WB while the branch resolves in EX — was derived from the pipeline's structure without first checking that the branch existed | absolute | **Open.** Implement `BC1F`/`BC1T`/`BC1FL`/`BC1TL`: four `Op` variants, decode from COP1 `rs = 0o10` with bits 17:16 as `nd:tf`, `is_likely` for the `*L` pair, and the condition plumbed into `execute` (which today has no access to `FCSR`). The branch-likely nullification machinery already exists. **Then** re-check the hazard: the same trace shows `FCSR.C` committing two cycles after an immediately-following branch would reach `EX`, so an interlock is likely still needed — but that is a second question, and the stall length must come from the trace, not from reasoning about which latch holds what |
