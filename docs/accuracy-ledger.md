...
| R-13 | Triangle **texturing** (T-33-004 PR-B 2b-texture) samples the tile per pixel via `decode_texture` + `interpolate_st` + `fetch_texel`, but only on the **non-perspective** coordinate path (`no_perspective_divide` — the integer part of the interpolated `s16.16`); the **perspective divide** (`perspective.h`'s 64-entry reciprocal LUT applied as `(s·rcp) >> shift` per pixel) and the exact **tile shift/clamp/mask** for triangle coordinates (as opposed to the copy-mode `wrap_coord` used by texture rectangles) are not yet modelled | The non-perspective path and the sampler wiring are the tractable first step (the flat-coordinate case is scale-independent, so a textured triangle can be validated end-to-end without the divide); the perspective LUT is a precision-critical port best hand-verified in isolation, and the triangle coordinate wrap/clamp is a combinatorial surface for the conformance fuzz | absolute — a coverage boundary, not a fitted constant | **Open.** The decode → interpolate → `fetch_texel` → combine → write path is validated by a textured-triangle test sampling a loaded RGBA16 texel (a flat coordinate, scale-independent). The perspective divide and the exact coordinate wrap/clamp land in the perspective slice and are validated against the ParaLLEl-RDP conformance vectors (T-33-005) |
