...
| R-13 | Triangle **texturing** (T-33-004 PR-B 2b) samples the tile per pixel via `decode_texture` + `interpolate_st` + `fetch_texel`, with **both** the non-perspective path and the **perspective divide** (the 64-entry `perspective.h` reciprocal LUT + normalisation shift + out-of-bounds saturation + `w <= 0` carry, gated on `persp_tex_en`) now implemented; what remains is the exact **tile shift/clamp/mask** for triangle coordinates (as opposed to the copy-mode `wrap_coord` used by texture rectangles) and the LOD/mip and second-texel (`texel1`, 2-cycle) path | The non-perspective path and the sampler wiring are the tractable first step (the flat-coordinate case is scale-independent, so a textured triangle can be validated end-to-end without the divide); the perspective LUT is a precision-critical port best hand-verified in isolation, and the triangle coordinate wrap/clamp is a combinatorial surface for the conformance fuzz | absolute — a coordinate-scale divergence, oracle-observed (not a fitted constant) | **Open — a divergence is now PINNED against Angrylion.** The decode → interpolate → `fetch_texel` → combine → write path is validated internally by a textured-triangle test sampling a loaded RGBA16 texel (a flat coordinate, scale-independent) and by the copy-mode `Texture Rectangle` round-trip — but `interpolate_st` had **never** been checked against the oracle. The new `tex_tri_16` conformance vector (a per-pixel S gradient over an 8-texel ramp, texel0 passthrough) now does, and it **diverges**: Angrylion loads the ramp into TMEM correctly (its 16-bit `tmem_formatting` places `tmemidx0 = 0xF801`), RustyN64 samples texel 0 at pixel (2,1) → `0xF801`, but Angrylion's golden is `0x0000` — its sampler indexes a different (unloaded) texel for the same S. Suspected cause: `interpolate_st` takes the integer texel as `v >> 16` (treating the coefficient as a plain s16.16 integer), whereas the RDP's texel coordinate is s10.5-based, so a given `dx.S` walks texels at a different rate. The vector is committed **`#[ignore]`d** to pin the divergence (the R-14 precedent); root-causing the exact scale (a coordinate sweep / Angrylion sampler trace) and the fix are the next step. The v2 `.rvec` **preload** plumbing this needed — a texture placed in RDRAM before the command list, read identically by both renderers — is itself verified (an all-white texture renders white through Angrylion). The perspective divide and the exact coordinate wrap/clamp are validated by further conformance vectors (T-33-005) |
