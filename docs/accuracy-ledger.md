...
| R-10 | The colour combiner (T-33-002) models the common inputs (combined, texel0/1, primitive, shade, environment, one, zero, and the C-slot alpha taps); the **exotic inputs** — noise, LO | These inputs need the LOD pipeline (mip level fraction), the key/convert registers (`Set Key`/`Set Convert`), and a noise source, none of which exist yet; they appear in a small mi | absolute — a coverage boundary, not a fitted constant | **Partially resolved (2026-07-25) — see the RESOLUTION below.** The `(A − B) * C + D` arithmetic (the `special_expand` asymmetric 9-bit fold, the `+0x80`-before-`>>8` rounding, D a **Full record: [R-10](residuals/R-10.md).** |
| R-13 | Triangle **texturing** (T-33-004 PR-B 2b) samples the tile per pixel via `decode_texture` + `interpolate_st` + `fetch_texel`, with **both** the non-perspective path and the **persp | The non-perspective path and the sampler wiring are the tractable first step (the flat-coordinate case is scale-independent, so a textured triangle can be validated end-to-end with | absolute — real texture-path modelling gaps, oracle-confirmed | **RESOLVED (2026-07-23) — see the RESOLUTION at the end of this cell. **Full record: [R-13](residuals/R-13.md).** |
...
| R-5 | **[HISTORICAL BASELINE — SUPERSEDED by the dated RESOLUTIONs in the disposition column, which are the current normative answer. | The framebuffer→RGBA8 pixel conversion is exact and cited: the pixel *format* is selected by `VI_CTRL.TYPE[1:0]` (N64brew *Video Interface* §VI_CTRL — 2 = RGBA5551, 3 = RGBA8888), | absolute — a resampling/filter geometry choice, not a timing interval | **Open.** Byte-exact for a 1:1, unfiltered scan of a framebuffer whose width matches `VI_WIDTH` — which is what the FILL pipeline produces and the T-31-004 unit tests pin. **Full record: [R-5](residuals/R-5.md).** |
...
| R-18 | A **commercial ROM boots and executes real code but does not reach video** (Phase 5 capstone). | The retail OS-boot runtime the game waits on is not yet modelled. | absolute — a coverage boundary across subsystems, not a fitted constant or a timing interval | **Open — characterised, not a regression.** The committable Phase 5 gate (n64-systemtest cart/PIF/SI, save round-trips, homebrew boot) is met; the commercial capstone is asserted a **Full record: [R-18](residuals/R-18.md).** |
...
| R-19 | **The emulator hangs on the n64-systemtest test `TLB: Execute mapped branch with a non-mapped delay slot`** — a mapped branch whose delay slot lies in a page not currently in the T | **The committed `systemtest` gate masks it**: `tests/systemtest.rs` asserts Phase-1 *category* `Failed: 0` (those results are captured before test 917) and witnesses `started > 0`, | absolute — a hang is a coverage boundary, not a fitted constant | **Open — fully characterised, not yet root-caused. **Full record: [R-19](residuals/R-19.md).** |
