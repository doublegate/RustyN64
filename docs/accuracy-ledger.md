...
| R-3 | FILL-mode `Fill Rectangle` (T-31-003) rasterises the rectangle with an **inclusive lower-right pixel** — floor the upper-left, and draw through the pixel that *contains* the lower-right coordinate; in FILL/COPY mode the low two bits of `yl` are forced set before the shift (`yl \| 3`) so the final scanline fills whole. The **scissor** clips separately with an **exclusive** lower-right | The N64brew wiki says only "upper-left rounded down, lower-right rounded up"; the exact edge behaviour is what the Angrylion oracle pins, and it is *inclusive of the lower-right pixel* (a rect `(0,3)-(1,4)` draws columns 0 **and** 1), plus the FILL/COPY `yl \| 3` (Angrylion `rasterizer.c` `rdp_fill_rect`). The earlier `(coord + 3) >> 2` half-open span dropped the boundary row/column — a realisation of the cited prose that disagreed with the hardware. Read from the oracle's output, not computed | absolute — a rasterisation geometry rule, not a timing interval, so the differential/re-phasing test is N/A | **Closed for the integer-coordinate FILL rect (oracle-validated).** The inclusive lower-right + `yl \| 3` rule is now pinned bit-for-bit against Angrylion by the **seeded-fuzz corpus** (`tests/vectors/fuzz/`, 48 random FILL rectangles sweeping colour/size/position with a full-image scissor, all byte-exact) and a mutation-checked unit test (`fill_rectangle_lower_right_edge_is_inclusive`). The fuzz gate **found** the pre-fix off-by-one. Still unverified: **sub-pixel** (fractional-coordinate) rect edges, which the whole-pixel fuzz does not exercise; and the **scissor** lower-right rounding is a *separate* open item (**R-15**). No n64-systemtest assertion drives the render path (count unchanged at 93) |
| R-15 | The **scissor** lower-right bound in FILL mode may be **inclusive** of its boundary pixel, not exclusive as `fill_rectangle` currently clips it (`(coord + 3) >> 2`, a half-open bound) | An exploratory scissor-clip fuzz batch (a random scissor sub-rect per FILL rectangle) diverged on 7/48 vectors, each with the R-3 signature — RustyN64 drew nothing at the scissor's lower-right edge where Angrylion filled (e.g. a scissor `xl = 7.0` kept column 7). This mirrors the R-3 rectangle-edge fix, but the isolating probe conflates the rectangle's own inclusive edge with the scissor's, and the existing `fill_rectangle_is_clipped_to_the_scissor` unit test asserts an **exclusive** scissor that was self-authored and never oracle-checked — so the true rule needs a clean probe where the scissor (not the rectangle) determines the edge, plus reconciliation of that test. Angrylion's `rasterizer.c` clips scanlines by `yllimit = min(rect_yl, clip.yl)` then renders `[yhlimit >> 2, yllimit >> 2]`; the exact inclusivity is what a dedicated probe must pin | absolute — a rasterisation geometry rule, oracle-observed but not yet isolated | **Open — deferred to a dedicated PR.** The R-3 fix (rectangle edge) stands independently and is validated by a full-image-scissor corpus. The scissor-clip sweep is not committed until the rule is isolated and the conflicting unit test reconciled, to avoid baking in a guess |
