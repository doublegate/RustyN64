...
| R-12 | The Z-buffer machinery (T-33-004 **PR-A**) implements the depth **codec** (`z_compress`/`z_decompress`, `dz_compress`/`dz_decompress`) and the per-pixel **`depth_test`** (all four Z modes, coplanar/precision-factor handling), plus the depth-source commands (`Set Depth Image` 0x3E, `Set Primitive Depth` 0x2E); the **Z-buffer RDRAM read/write** (the 18-bit-per-pixel "hidden bit" storage format), the **coverage accumulator** at edges, and the **per-pixel routing** of combiner→blender→depth in the triangle rasteriser are not yet wired | These are pure, oracle-verifiable functions and can land — and be tested — ahead of the storage format and the pipeline integration, which are the larger, riskier surface (the hidden-bit Z packing needs the RDRAM 9th-bit research, and the routing is the flat-fill→per-pixel rewrite that also closes R-9). Splitting keeps each PR reviewable (the project's split-large-tickets rule) | absolute — a coverage boundary, not a fitted constant | **Open.** The codec is validated by hand-computed boundary values and a `z_compress ∘ z_decompress` round-trip; `depth_test` by observable occluding-vs-occluded pairs per Z mode (six `rdp` unit tests). `depth_test`/the codec have **no runtime caller** yet, so the oracle stays **93**. The storage format, coverage, and routing land in **PR-B** and are validated against the ParaLLEl-RDP conformance vectors (T-33-005) |
...
| R-9 | The triangle rasteriser (T-33-001) **flat-fills** with the FILL-mode colour and reduces each edge to whole pixels (`>> 16`); the bit-exact **sub-pixel coverage** (ParaLLEl-RDP's `quantize_x` sticky-bit edge rounding and the `do_offset` last-subpixel latch) and the **shade/texture/Z attribute interpolation** (`d*de`/`d*dx`/`d*dy`) are not modelled yet — the 0x09–0x0F variants fill flat, their coefficient words length-consumed only | Landing the edge-walk and the s11.2/s11.16 decode first, validated against a hand-computed triangle, is the tractable foundation; the sub-pixel coverage and the combiner-driven colour are a combinatorial surface best pinned by the conformance fuzz (T-33-005). A 1-/2-cycle triangle is coloured by the combiner/blender, which do not exist yet, so FILL-mode flat fill is the honest first target | absolute — a coverage boundary, not a fitted constant | **Open.** The flat fill is validated pixel-for-pixel against a right-triangle golden (the edge-walk, `lmajor` selection, scissor, and fixed-point decode). The sub-pixel edge rule and attribute interpolation land with T-33-002…004 and are validated against the ParaLLEl-RDP conformance vectors (T-33-005) |
