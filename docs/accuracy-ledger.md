...
| R-19 | **The emulator hangs on the n64-systemtest test `TLB: Execute mapped branch with a non-mapped delay slot`** — a mapped branch whose delay slot lies in a page not currently in the TLB. Both the committed **base** ROM and the `--features timing` ROM stop dead there: `started = 917`, `emux_exited = false`, no test after it ever starts, at an 8×10⁹-tick budget (~2× a normal base completion). It is a genuine loop, not slowness. **The committed `systemtest` gate masks it**: `tests/systemtest.rs` asserts Phase-1 *category* `Failed: 0` (those results are captured before test 917) and witnesses `started > 0`, but never requires the ROM to run to `xioctl(EXIT)` — so a mid-suite hang is invisible (the failure mode engineering-lessons §2.2 warns about, one level up). The fetch-fault path itself computes `in_delay_slot` correctly (`pipeline.rs` ~L2061/2150, reverse-cascade-aware) and raises `TlbRefill` with `BD` set, so the defect is subtler — most likely the **exception-vector selection or the handler-return path for a delay-slot IFETCH miss**: the handler ERETs to `EPC` (the branch), re-executes it, re-faults the still-unmapped delay slot, and loops, because something in the `EPC`/vector/`EXL`-nesting is off for this specific case | absolute — a hang is a coverage boundary, not a fitted constant | **Open — localised, not yet root-caused. Blocks the `timing` suite from completing (so it blocks the clean `M` measurement, C-1).** Next step: capture the PC/exception trace during the loop (arm on the `Running TLB: Execute mapped branch…` marker per engineering-lessons §3.3), compare our `EPC`/`Cause`/vector against n64-systemtest's `test_miss_exception` expectations (`ref-proj/.../tlb/exceptions.rs`), fix, then add ROM-completion (`emux_exited`) as a *witness* to `tests/systemtest.rs` so this class of hang can never hide again. Discovered 2026-07-24 during the Stage-C/D timing work |
