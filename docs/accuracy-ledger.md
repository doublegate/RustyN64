...
| R-5 | VI scan-out (T-31-004) is a **1:1 copy** — `VI_X_SCALE`/`VI_Y_SCALE` resampling and the AA / divot / de-dither post-filters are not applied, and the height is derived directly from `VI_V_VIDEO`'s active half-lines rather than from the scale-accumulated framebuffer walk | The framebuffer→RGBA8 pixel conversion (RGBA5551 5→8 bit expansion, RGBA8888 copy) is exact and cited (N64brew *Video Interface*, *Video DAC*); what is deferred is the geometric resampling (the VI samples the framebuffer at `VI_X_SCALE`/`VI_Y_SCALE` steps, accumulating a sub-pixel offset per line) and the analog-emulation post-filters, which only matter once scaled or anti-aliased content is scanned | absolute — a resampling/filter geometry choice, not a timing interval | **Open.** Byte-exact for a 1:1, unfiltered scan of a framebuffer whose width matches `VI_WIDTH` — which is what the FILL pipeline produces and the T-31-004 unit tests pin. Scaling and the post-filters **will be** validated bit-for-bit against Angrylion via the ParaLLEl-RDP fuzz suite / VI golden frames (Sprint 3), and superseded here if they diverge — this entry stays open until then |
