...
- **The `test-roms` / `commercial-roms` / per-crate `std` feature flags still gate zero code** —
  no `cfg(feature = …)` exists for them yet, so `--features test-roms` runs the same tests as a
  bare `cargo test`.
