...
reported and `FCSR.RM` cannot be honored. The module is verified bit-for-bit against those same
operators in round-to-nearest over ~100k cases — they are the independent oracle, which is why it
implements *IEEE* behavior and leaves the VR4300's refusal to produce subnormals as a separate
layer.
...
- **NaN classification is INVERTED from IEEE-754:2008** — significand MSB *set* means *signaling*,
...
- **Cache ↔ DMA coherency is not modeled.** Cart/RSP DMA writes land in RDRAM behind the cache.
