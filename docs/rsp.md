...
the sequential address.** The redirect the branch already latched still wins, so
`beq` (taken) into a `break` leaves PC at the target (`0x1C` for the suite's
`beq r0,r0,+6`), while the same `break` after an *untaken* branch leaves it
sequential (`0x8`). Reading the sequential value unconditionally passes the
plain `BREAK` and untaken cases and fails only the taken one.
...
(`ref-docs/research-report.md` §3). The tables themselves are generated by exact
integer arithmetic rather than stored as literals — a deliberate departure from
that rule, argued in accuracy ledger **C-31**.

**Implemented**: `VRCP`, `VRCPL`, `VRCPH`, `VRSQ`, `VRSQL`, `VRSQH`, `VMOV`,
`VNOP`. These are *single-lane* instructions: they read one lane of `vt`
(selected by the element field), write one lane of `vd` (selected by `de`, which
occupies the `vs` position), and set the whole accumulator low slice from the
broadcast source — including `VRCPH`, which computes nothing. `VNOP` is the
exception that touches nothing at all.

The H/L pairs stage a 32-bit operand through `DIVIN`/`DIVOUT`, and the `pending`
flag is load-bearing: it separates *"a high half was staged by the preceding
instruction"* from *"there is a stale value in the latch"*. Without it an `L`
issued on its own silently consumes whatever the last `H` left behind, making
the result depend on unrelated earlier code. `VRCPH` also hands back the
**previous** result's high half rather than anything derived from its own
operand — returning the staged value instead looks plausible and produces
