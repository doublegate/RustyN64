...
///
/// **17:12, not 16:12.** Bit 17 is `Cause.E`, Unimplemented Operation: part of
/// `Cause` despite having no `Enable` bit and no sticky `Flags` twin, which makes
/// this mask the *only* thing that ever clears it. A narrower 16:12 mask left bit
/// 17 permanently set once raised — a shipped bug in this file's history, and the
/// reason the constant lives here rather than being written out at each site. It
/// had reached four copies.
///
