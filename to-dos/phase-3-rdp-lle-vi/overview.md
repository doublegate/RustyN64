...
rasterizes it through the full per-pixel pipeline — texture fetch, the color combiner, the
...
rasterizes through the texture / combiner / blender / coverage pipeline with VI scan-out, proven
...
- [~] The color combiner implements the full mux (`Set Combine Mode`, 0x3C) for both cycles —
...
- [x] The synchronization commands behave: `Sync Load` (0x26), `Sync Pipe` (0x27),
...
- The rasterizer: edge-walk, texture, combiner, blender, Z/coverage.
...
- [Sprint 3 — The color combiner, the blender, Z/coverage, and the fuzz 0-diff](sprint-3-pipeline.md) —
...
Phase 2 complete: without the LLE RSP there is no real command list to rasterize. The
...
- **Angrylion is license-poisoned** — it is the natural thing to read while chasing a
  bit-exactness failure, and it is non-commercial MAME-licensed. Compare outputs, never source
  (`ref-proj/README.md`).
- **Performance will look alarming** — a per-pixel software rasterizer at N64 fill rates is
  slow, and the temptation is to optimize before it is correct. Mitigated by ADR 0002's ordering:
  correctness first, acceleration later and validated.

## Reference docs

- [docs/rdp.md](../../docs/rdp.md) — the rasterizer spec.
