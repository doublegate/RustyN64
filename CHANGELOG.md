...
- Evidence standard held: on the same run Ocarina scored 62,963 lit pixels and,
  rendered and viewed, is **pure noise**. Pixel count alone cannot tell the two
  apart, so `screenshots/README.md` requires looking at the image.

### Fixed — the capstone measured a buffer no user sees

- `commercial_boot` counted lit pixels through the superseded 1:1 `Bus::scanout`
  rather than `Bus::scanout_scaled`, the path the frontend actually presents, so
  every number it reported was off by the real `VI_X_SCALE` upscale (~2x).
- `Bus::scanout_scaled`'s rustdoc still claimed it was "not yet wired into the
  frontend" — false since #158, which wired it.
...
### Added — the frontend opens zipped ROMs

- **`File -> Open ROM` and the CLI now accept a `.zip`.** ROM sets are
  distributed as one `.z64` per `.zip`, so requiring a manual unpack was friction
  with no upside. New `rustyn64-frontend::romfile` module; native-only (the wasm
  build receives bytes from the browser file API, never a path).
- **The container is sniffed by magic (`PK\x03\x04`), not by extension** — the
  same rule the byte-order detection already follows, and for the same reason. A
  zip named `game.z64` is unwrapped; a raw ROM named `game.zip` is read verbatim.
- Exactly one `.z64`/`.n64`/`.v64` member (matched case-insensitively) is
  extracted. Zero is an error; **more than one is an error naming the
...
- **Alpha-compare now gates the pixel write** (`Set Other Modes` bit 0) on **both**
  ...
  `dx.A = 0x30`, against a `0x80` threshold, so low-alpha columns are killed) and
  `alpha_compare_z_16` (the same on a z-suffixed triangle) — plus a boundary unit
  test. 16 conformance vectors now pass (+ 1 ignored WIP). Still deferred (ledger
  R-11): the dithered-threshold variant (`dither_alpha_en`).
...
- The new `tex_tri_16` vector (an 8-texel ramp sampled across a triangle, texel0
  passthrough) is the first to drive `interpolate_st` against Angrylion, committed
  **`#[ignore]`d**. It pins a **real** divergence that RustyN64 does not yet model
  (settled by direct Angrylion instrumentation, after two retracted mis-diagnoses —
...
[0.6.0]: https://github.com/doublegate/RustyN64/releases/tag/v0.6.0
[0.5.0]: https://github.com/doublegate/RustyN64/releases/tag/v0.5.0
[0.4.1]: https://github.com/doublegate/RustyN64/releases/tag/v0.4.1
[0.4.0]: https://github.com/doublegate/RustyN64/releases/tag/v0.4.0
[0.3.0]: https://github.com/doublegate/RustyN64/releases/tag/v0.3.0
[0.2.0]: https://github.com/doublegate/RustyN64/releases/tag/v0.2.0
